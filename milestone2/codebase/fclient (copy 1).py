...
def mkdirReq(newDirectory):
    mkdir = Filesystem(newDirectory) #didn't want to make new class called mkidr
    channel = grpc.insecure_channel(ip, options=(('grpc.enable_http_proxy', 0),))
    stub = account_pb2_grpc.ReplyStub(channel)
...
def run(): #directoryS
    ...
    sysCallOptions = sysCallOptions.split(" ")
    print(sysCallOptions)
    if sysCallOptions[0] == "ls":
        ...
        statReq()
    elif sysCallOptions[0] == "mkdir":
        print("Sending a mkdir request to the server.")
        mkdirReq(sysCallOptions[1])
        ...
        mknodReq()
    elif sysCallOptions[0] == "rmdir":
        print("Sending an rmdir request to the server.")
        rmdirReq(sysCallOptions[1])
